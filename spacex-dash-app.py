...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values = 'class', names = 'Launch Site', title = 'Success of launch per site')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df = filtered_df['class'].value_counts().reset_index()
        filtered_df.columns = ['class', 'count']
        fig = px.pie(filtered_df, values = 'count', names = 'class', title = f'Success of launch at {entered_site} site')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, payload_mass):
    ...
    if entered_site == 'ALL':
        ...
        fig.update_xaxes(range = payload_mass)
        return fig
